...
export function tryReadSerializedValue(
	value: undefined | SerializedConstant | Record<string, SerializedConstant>,
): undefined | SerializedConstant {
	if (value === undefined) {
		return undefined;
	}
	...
	if (typeIs(value, "table") && "type" in value) {
		return value as SerializedConstant;
	}
...
export function deserializeConstant(
	serialized: undefined | SerializedConstant,
	fallback: SupportedPrimitive,
): SupportedPrimitive {
	...
	if (
		typeIs(serialized, "number") ||
		typeIs(serialized, "string") ||
		typeIs(serialized, "boolean")
	) {
		return serialized;
	}
