...
// pnpm can print warnings (e.g. about project-level .npmrc auth settings) to
// stdout before the JSON payload; skip everything up to the first `[`.
function parsePnpmJsonOutput(output) {
	const jsonStart = output.indexOf("[");
	if (jsonStart === -1) {
		throw new Error(`Expected a JSON array from pnpm, got: ${output.slice(0, 500)}`);
	}
...
function getWorkspaceDirectories() {
	...
	const parsed = parsePnpmJsonOutput(output);
